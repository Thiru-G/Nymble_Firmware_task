...
def SerialConnection(e_text):           
    ...
    start_time = time.time()        #Marking Start of transmission
    write_read(e_text)  
    end_time = time.time()          #Marking end of transmission
...
#UI APP Class is built to provide UI using tkinter and TTK
class UIAPP:
    def __init__(self, master=None):         
        ...
        def Submittedfunc():            # Function occurs when Submit button is pressed
            e_text=entry1.get()         
        ...
        button1.configure(text='Submit', command = Submittedfunc)
